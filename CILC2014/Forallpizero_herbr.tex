...
considering\marginpar{added} the \DisjDatalog syntax accepted by
...
Notice that (C1) and (C2) are not in $\Forallpizero$\marginpar{added
\#11}
...
% Let us clarify our reduction by means of an example. Let us consider to this 
% purpose the \ForallpizeroDisjDatalog-formula 
% $\varphi=(\forall \nonpairin{x}{a})(\nonpairin{x}{b}) \wedge (\forall
% \pairin{x}{y}{a})(\pairin{x}{y}{b})$, which models the set-theoretic expression
% $a \subseteq b$.
% The constants occurring in it are $V=\{a, b\}$. Now, let $T$ be a set of constants such that $|T|=2\cdot|V|$ and $T\cap V=\emptyset$, 
% for example $T=\{t_1, t_2, t_3, t_4, t_5, t_6\}$. An equisatisfiable
% $\ForallpizeroDisjDatalog$-formula $\Theta_{\cup}$ can be constructed as
% follows:
% \[
% \Theta_{\cup} \defAs \chi^{(V,T)} \wedge \psi_{\cup} = \regf \wedge \exonef
% \wedge \extwof \wedge \eqf \wedge \psi_{\cup}
% \]
% where $T$ is a set of constants such that $|T|=2\cdot|V|$ and $T\cap
% V=\emptyset$, for example $T=\{t_1, t_2, t_3, t_4\}$, 
% $\regf$ and $\exonef$ are those defined above, $\extwof$ and $\eqf$ are as
% follows
% \begin{align*}
%    \extwof \equivAs {} & \neg(\seteq{a}{b}) \rightarrow 
%    \distby{x}{y}{a} \vee \distby{y}{x}{a} 
%    \distby{x}{y}{b} \vee \distby{y}{x}{b} 
%    \distby{x}{y}{a} \vee \distby{y}{x}{a} 
%    \distby{x}{y}{a} \vee \distby{y}{x}{a} 
%    \distby{x}{y}{a} \vee \distby{y}{x}{a} 
%    \vee \distPby{x}{y}{z}{v} \vee
%    \distPby{y}{x}{z}{v} bigwedge\limits_{x,y \in V}
%    \big( \neg(\seteq{x}{y}) \rightarrow \distS^{(V,T)}(x,y) \vee 
% \distPS^{(V,T)}(x,y) \big)  \\
%    & {~} \wedge (\forall x,y,z)\big(\distby{x}{y}{z} \rightarrow 
% \nonpairin{z}{x}\big) \\
