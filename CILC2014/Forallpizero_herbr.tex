...
our attention on the \emph{query answering} problem,  
...
on the one hand, it provides a way to implement with small efforts a solid
...
$\psi$ two formulae of a first order language $\Lang$. We say that $\varphi$ entails $\psi$, and
write $\fomodels{\varphi}{\psi}$, if and only if $\varphi \rightarrow \psi$ is
valid.
In other words, $\varphi$ entails $\psi$ if and only if $\psi$ is evaluated to
$\true$ by every pair $(\inter, A)$ evaluating $\varphi$ to $\true$
 as well.
...
\emph{query answering}, which is used to extract the \emph{implicit knowledge}
...
consider the \DisjDatalog syntax accepted by
...
knowledge bases.
A \emph{conjunctive query} is a finite conjunction of atoms. Notice that a conjunctive query is allowed to contain
free variables. 
...
\end{array}
\]
The only \emph{answer} $\sigma$ for the query $\mathbf{Q}$ and the formula
$\varphi$, i.e. the only substitution such that $\varphi 
...
those substitutions involving only variables occurring in the query 
and constants occurring in the formula. 
...
two $\Forallpizero$-sentences $\varphi$ and $\psi$, we say
...
Theorem \ref{FORALLPIZEROSAT} and Corollary \ref{FINALCOR} concern the satisfiability problem and have consequences
on other problems related to satisfiability such as, for example, the query
answering problem. This is shown by the following corollary.
...
semantics, i.e. if and only if $\fomodels{\chi^{(V,T)}\wedge\varphi}{\sigma\mathbf{Q}}$.
\end{corollary}
\begin{proof}
Entailment can be reduced to
(un)satisfiability and thus $\vmodels{\varphi}{\sigma\mathbf{Q}}$ iff $\varphi
\wedge \neg (\sigma\mathbf{Q})$ is unsatisfiable. But
$\varphi \wedge \neg (\sigma\mathbf{Q})$ is equivalent to
