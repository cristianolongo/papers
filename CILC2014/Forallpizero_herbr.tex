...
reduction,\marginpar{ Issue 26 the reference to
"applications of techniques and results devised in the context of logic
programming" is too vague, and it should be further investigated or deleted.} some complexity 
...
let $\mathcal{Q}=q_1 \wedge \ldots q_n$ be a
conjunctive query, i.e.
a finite conjunction of atoms $q_1 \wedge \ldots q_n$, let $V=\Consts(\varphi)
\cup \Consts(\mathcal{Q})$, let $T$ be any set of constants disjoint from $V$ such that $|T|=2\cdot|V|$ and let $\sigma$
be a substitution which associates a constant in $V$ to every variable of
$\mathcal{Q}$. Then $\sigma$ is an answer for $\mathcal{Q}$ and $\varphi$
with respect to the set-theoretic semantics, i.e.
$\vmodels{\varphi}{\sigma\mathcal{Q}}$, if and only if $\sigma$ is an answer for
$\mathcal{Q}$ and $\chi^{(V,T)}\wedge\varphi$ with respect to the Herbrand
semantics, i.e. if $\fomodels{\chi^{(V,T)}\wedge\varphi}{\sigma\mathcal{Q}}$.
\end{corollary}
\begin{proof}
In general, entailment can be reduced to
(un)satisfiability. Indeed, $\vmodels{\varphi}{\sigma\mathcal{Q}}$ iff $\varphi
\wedge \neg (\sigma\mathcal{Q})$ is unsatisfiable. But
$\varphi \wedge \neg (\sigma\mathcal{Q})$ is equivalent with
...
$\mathcal{Q}$. But, for Corollary \ref{FINALCOR}, $\varphi \wedge
...
unsatisfiable if and only if $\varphi \fomodels \sigma \mathcal{Q}$.
\end{proof}
